...
class $modify(endscreen,EndLevelLayer){
	...
	void playEndAnimation(){
		...
		*/
		...
		if (auto mhCheatIndicator = this->m_mainLayer->getChildByType<CCLabelBMFont>(0)){
			mhCheatIndicator->setPosition({-500,winSize.height*0.719f});
			mhCheatIndicator->runAction(CCWait(2.8, CCEaseExponentialOut::create(CCMoveTo::create(1.3,{40,winSize.height*0.719f}))));
		}
		if (auto mhVersion = this->m_mainLayer->getChildByType<CCLabelBMFont>(1)){
			mhVersion->setPosition({-500,winSize.height*0.688f});
			mhVersion->runAction(CCWait(2.8, CCEaseExponentialOut::create(CCMoveTo::create(1.3,{60,winSize.height*0.688f}))));
		}
